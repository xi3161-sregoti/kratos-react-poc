...
function Login() {
    ...
    //   .then(({ data }) => {
    //       console.log("Browser login flow started", data);
    //       setFlow(data)
    //   })
    // }, [flow])
    ...
    const handleSubmit = (event) => {
        ...
        ory.createBrowserLoginFlow()
            .then(({ data }) => {
                console.log("Browser login flow started", data);
                // The data varialble has ui.nodes property which is an array of object. Iterate over it & return attributes.value where attributes.name == "csrf_token"
                let csrfToken = "";

                if (data.ui && data.ui.nodes) {
                    // Iterate over the array of nodes
                    data.ui.nodes.forEach(node => {
                        // Check if the current node has the attribute named "csrf_token"
                ...
                const url = `/self-service/login?flow=${data.id}`
